...
DEFAULT_OUTPUT_QUEUE = "product_revenue_by_month"      # batches agregados por (year, month, item_id)
YEARS = {2024, 2025}
...
# Estado en memoria
# ------------------------------
# agg[(year, month, item_id)] = sum_subtotal
agg = defaultdict(float)
_last_client_id = 0  # para completar el header saliente
...
def handle_batch(batch_bytes: bytes):
    ...
    for row in batch.rows:
        ...
        if item_id is None or subtotal is None or created_at is None:
            continue
        ...
        try:
            ...
            val = float(str(subtotal).replace(",", "."))

        agg[(year, month, item_id)] += val
        processed += 1
...
def build_output_chunk() -> bytes:
    ...
    (item_id, sum(subtotal) en (year, month)).
    - transaction_id: ""
    - item_id: item
    - quantity: 0 (no aplica en este agg)
    - subtotal: suma mensual (revenue)
    - created_at: primer día del mes (para preservar año/mes)
    """
    rows = []
    for (year, month, item_id), revenue in agg.items():
        rows.append(
...
def main():
    ...
    # Consume hasta que termine o reciba señal
